  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {movies.map((movie) => {
        ...
        return (
          <div key={movie.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 hover:scale-105">
            <div className="aspect-3/4 bg-gray-200 relative overflow-hidden">
              ...
              <div className={`absolute inset-0 ${getPosterOverlayColor(averageRating, movie.status)}`}></div>
              
              {/* Type badge */}
              <div className="absolute top-3 left-3">
                <span className={`px-2.5 py-1 rounded-full text-xs font-bold shadow-lg ${getTypeColorScheme(movie.type)}`}>
              ...
              <div className="absolute bottom-3 left-3">
                <span className={`px-2.5 py-1 rounded-full text-xs font-bold shadow-lg ${getStatusColorScheme(movie.status)}`}>
              ...
              <div className="absolute bottom-3 right-3">
                <div className={`px-2.5 py-1 rounded-full text-xs font-bold shadow-lg flex items-center space-x-1 ${getViewCountColorScheme(movie.view)}`}>
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="p-4">
              <div className="font-bold text-gray-800 mb-1 line-clamp-2 text-lg">{movie.title}</div>
              <div className="text-sm text-gray-500 mb-3 line-clamp-1">{movie.originalName}</div>
              ...
              <div className="space-y-2 text-sm mb-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-bold ${getLangColor(movie.lang)}`}>
                    {movie.lang}
                  </span>
                </div>
              ...
              <div className="flex space-x-2">
                <button 
                  ...
                  className="flex-1 bg-linear-to-r from-blue-500 to-blue-600 text-white py-2.5 rounded-lg text-sm font-medium hover:from-blue-600 hover:to-blue-700 transition-all duration-200 flex items-center justify-center space-x-1"
                  title="Chi tiết"
                >
                ...
                <button 
                  ...
                  className="bg-linear-to-r from-red-500 to-red-600 text-white px-3 py-2.5 rounded-lg text-sm font-medium hover:from-red-600 hover:to-red-700 transition-all duration-200"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
